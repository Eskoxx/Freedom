...
class _ProxyServer(HTTPServer):
    def __init__(self, target_url: str, session: CurlSession):
        self._target_url = target_url.rstrip("/")
        self._session = session
        super().__init__(("127.0.0.1", 0), _ProxyRequestHandler)


def _start_proxy(target_url: str, session: CurlSession) -> tuple[str, HTTPServer]:
        ...
        try:
            ...
            for prov in providers:
                for q in prov.get("qualities", []):
                    qh = q.get("height", 0)
                    token = q.get("token", "")
                    if token and qh and qh <= target_quality:
                        if chosen_token is None or qh > chosen_quality.replace("p", "").isdigit() and int(chosen_quality.replace("p", "")) < qh:
                            chosen_token = token
                            chosen_quality = q.get("name", f"{qh}p")
                    if not chosen_token and token:
                        chosen_token = token
                        chosen_quality = q.get("name", "unknown")

            if not chosen_token:
                chosen_token = providers[0].get("qualities", [{}])[0].get("token", "")
                chosen_quality = providers[0].get("qualities", [{}])[0].get("name", "unknown")
            if not chosen_token:
                return None
