...
class _VixHlsProxy:
    ...
    vix-content.net throttles per connection, so mpv's single sequential HLS
    demuxer connection gets capped (~600kbps) while a browser's ~6 parallel
    connections are fast. This proxy downloads segments concurrently into a
    local cache and serves mpv from 127.0.0.1, restoring that parallelism.
    """
    ...
    def _build(self, master_url: str) -> None:
        ...
        for line in lines:
            if line.startswith("#EXT-X-MEDIA"):
                ...
                stripped = line.strip()
                rewritten.append(vid_routes.get(stripped, line))
            else:
                rewritten.append(line)
...
def _make_proxied_stream(master_url: str, referer: str) -> Optional[StreamSource]:
    try:
        proxy = _VixHlsProxy(master_url, referer)
